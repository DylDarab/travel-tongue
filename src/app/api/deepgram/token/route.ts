...
export async function GET() {
  try {
    ...
    if (error || !result?.access_token) {
      return NextResponse.json({ error: error ?? 'grantToken failed' }, { status: 500 })
    }

    return NextResponse.json({ accessToken: result.access_token, expiresIn: result.expires_in ?? 30 })
  } catch (error) {
